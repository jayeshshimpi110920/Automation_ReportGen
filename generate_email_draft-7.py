...
def calc_rem_date(first_date, severity):
    """Returns (rem_date_str, is_overdue)."""
    sev = severity.lower()
    if sev == "critical":
        return "N/A", False
    if sev == "info":
...
# ── Fuzzy match ───────────────────────────────────────────────────────────────
def fuzzy_match(a, b):
    """True if similarity >= FUZZY_THRESHOLD (case-insensitive)."""
...
def load_tracker(xlsx_path):
    """
    Reads xlsx, uses FIRST ROW as header (column names).
    Returns list of dicts with keys:
      vuln_name, vuln_name_lower, first_identified (date),
      severity, remediation, assessment
    """
    ...
    # Find Vulnerability Tracker sheet or use active
    sheet = None
    ...
    def col(keyword):
        """Find column index by partial case-insensitive match on header."""
        kl = keyword.lower()
    ...
    missing = []
    if ci_vuln   is None: missing.append("Vulnerability Name")
    if ci_first  is None: missing.append("First Identified Date")
    if ci_sev    is None: missing.append("Vulnerability Severity")
    if ci_rem    is None: missing.append("Remediation Status")
    if ci_assess is None: missing.append("Assessment Type")
    ...
    for row in rows[1:]:
        try:
            ...
            # Normalise assessment → SAST / SCA / IaC
            assess = ""
            al = assess_raw.lower()
...
# ── Tracker lookup with fuzzy match ──────────────────────────────────────────
def tracker_lookup(tracker_rows, vuln_name, section):
    """
    Returns best matching tracker row or None.
    Priority: same section exact > same section fuzzy > any section exact > any section fuzzy
    """
    ...
    for row in tracker_rows:
        rl        = row["vuln_name_lower"]
        is_exact  = (rl == vl)
        is_fuzzy  = fuzzy_match(vuln_name, row["vuln_name"]) if not is_exact else False
        same_sec  = (row["assessment"] == section)

        if is_exact and same_sec  and same_sec_exact is None:  same_sec_exact = row
        if is_fuzzy and same_sec  and same_sec_fuzzy is None:  same_sec_fuzzy = row
        if is_exact and not same_sec and any_sec_exact is None: any_sec_exact = row
        if is_fuzzy and not same_sec and any_sec_fuzzy is None: any_sec_fuzzy = row

    return same_sec_exact or same_sec_fuzzy or any_sec_exact or any_sec_fuzzy
...
def parse_report(xml_path, tracker_rows):
    ...
    xml_seen  = {"SAST": set(), "SCA": set(), "IaC": set()}  # lowercased vuln names

    # SAST/IaC groups: key=(sec, vuln_name_lower, severity)
    sast_iac_groups = {}
    ...
    for finding in root.iter("finding"):
        ...
        if sec == "SCA":
            ...
            if first_result is not None:
                for node in (first_result.findall(".//Additional-value") +
            ...
            if t is None:
                # CASE: Not in tracker → New
                status           = "New"
                rem_str, overdue = calc_rem_date(fs_date, severity)

            elif t["remediation"].strip().lower() == "fixed":
                # CASE: Fixed in tracker but reappeared in XML → New
                status           = "New"
                rem_str, overdue = calc_rem_date(fs_date, severity)

            else:
                # CASE: Active in tracker → Active, use tracker First Identified Date
                status           = "Active"
        ...
        # ── SAST / IaC: group by (vuln_name, severity), sum instances ─────────
        else:
    ...
    # ── Fixed table ───────────────────────────────────────────────────────────
    # Tracker Active + NOT found in XML (fuzzy) → Fixed
    fixed = {"SAST": [], "SCA": [], "IaC": []}

    for row in tracker_rows:
        if row["remediation"].strip().lower() != "active":
            # Tracker Fixed + not in XML → ignore completely
            continue
        ...
        # Check if this tracker vuln exists in XML (fuzzy match against xml_seen)
        found_in_xml = any(
